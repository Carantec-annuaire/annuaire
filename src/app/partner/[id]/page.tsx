        ...
        <>
            ...
            <div className="flex flex-col items-start gap-y-1 p-2">
                ...
                <div className="text-xl text-slate-600">
                    {partenaire.champsAction}</div>
                <div className="text-xl text-slate-600 mb-4">
                    {partenaire.adresse}</div>

                {partenaire.mail && <Email email={partenaire.mail} />}
                {partenaire.telephone && <Phone phone={partenaire.telephone} />}

                {partenaire.site && (
                    <a href={partenaire.site} target="_blank" rel="noopener noreferrer" className="flex items-center">{partenaire.site} <ArrowUpRight className="h-4 w-4 mb-2" /></a>
                )}
